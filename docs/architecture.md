...
1. **录制**：`tmux pipe-pane -t <session>:0.0 'cat >> <log>'`（`output-broadcaster.ts`）。sink 命令经 `PIPE_SINK_CMD` 可配；在 Linux 环境（CI / 容器内测试）优先用 busybox cat——个别发行版把 `cat` 指向 uutils 实现，其行缓冲会造成输出成批延迟到达。
...
3. **attach replay**：新连接先注册 tap、再取 snapshot、最后 flush pending，保证字节序无空洞（`attachWithReplay`）。snapshot 两种模式：
   - **emulator 模式**（`TMUX_HUB_EMULATOR=1`）：server 侧 headless xterm + SerializeAddon 序列化当前屏幕 + 1000 行 scrollback，再追加 `ModeShadow.serializeModes()` 补发终端模式状态（DEC private modes：鼠标上报、光标可见性、滚动区域）——这些状态不在文本快照里，不补发会导致 attach 后鼠标 / 光标行为错乱。
   - **传统模式**：RIS（Reset to Initial State，转义序列 `\x1bc`，整屏重置）+ ring buffer 尾部字节，上限 `REPLAY_CAP_BYTES`（默认 256KB，防移动端解析器 OOM）；被截断时经 SSE 发 `replay_truncated`。
...
| `{kind:"wheel", ...}` | SGR(1006) 鼠标编码（xterm 标准鼠标上报协议，`mouse-encode.ts`，每 notch 一条，上限 20）→ `send-keys -l` |
...
- **删除去抖**：早期版本曾因 tmux 瞬时不可达把整张 managed session 表误清（PR #90/#92，本条即其根治方案）。现在 managed session 在 tmux 消失需连续 **3 个 poll** 确认才从 DB 删除并广播 removed；`list-sessions` 返回 "no server running" 判为**探测不确定**（serverReachable=false），跳过一切 prune。反向 self-heal：tmux 里活着但 DB 缺记录则自动补回。
...
`auth.ts` 对所有写操作类 endpoint（POST 等 mutating 请求）强制认证，按优先级尝试三层，得到的 identity 标签用于语音等按用户隔离的功能：
...
- **渲染**：xterm.js + CanvasAddon（延迟 50ms 加载，失败降级 DOM 渲染）；拦截 DECSCUSR（设置光标形状的转义序列），防止 TUI 应用改坏光标样式。
- **心跳/重连**：15s ping、5s 无 pong 判断线；指数退避 + jitter（500ms→30s，最多 8 次），之后 dead 态每 60s 探测；出站消息断线时排队（上限 65KB）重连后 flush。
- **本地预测 echo**：printable 字符本地即时回显掩盖网络延迟；检测到光标定位类转义序列或 alternate screen buffer（说明前台是 TUI 应用）即禁用本地预测，预测缓冲 2s 过期。

### 7.2 滚动体系

滚动恢复曾多轮返工（PR #87/#88/#91/#93），最终收敛为四个模块协作，核心不变量是 **client 本地 linesFromBottom (lfb) 是唯一真值**——server DB 里的滚动位置只作记录，不再参与恢复决策：

- `momentum-scroll.ts`：touch 拖动 + 惯性 fling（velocity EMA、行高量化）；滚动位置被本模块之外的代码改动时立即取消惯性动画；alt-screen TUI 下 drag 转 wheel ticks 转发给应用。
...
- **缓存策略**：Workbox precache app shell + 后台 revalidate；navigation 请求网络优先、离线降级；3xx 响应不缓存（防 CF Access OAuth 重定向被缓存造成回环）。
...
全部配置经 env 解析于 `config.ts`，按域分组；**下表变量名省略了统一前缀 `TMUX_HUB_`**（`GATE_INJECT_KEY` 除外，完整默认值见该文件）：
...
| 认证 | `SECRET_PATH` · `ADMIN_SECRET_PATH` · `GATE_INJECT_KEY`（无前缀）· `GATE_APP` |
...
- **unit**（53 文件）+ **integration**（20 文件）+ 源码同目录测试（4 文件）走 `bun test`，**E2E** 走 Playwright（6 个 spec：desktop/mobile/PWA/key-conformance/suggest/emulator-tui）。
- **生产状态强隔离**（同为 PR #90 事故的产物）：`tests/helpers/lint-no-prod-state.ts` 静态检查禁止测试裸建 `new ManagedSessionDb()`（会打开生产 DB），必须显式 temp 路径；`lint-no-default-socket.ts` 同理强制测试用独立 tmux socket（`-L`），杜绝测试触碰生产 tmux server 与 DB。
- 部署形态为双实例金丝雀：**dogfood 实例**始终跟随 main 最新 commit（作者日常自用即验证），**prod 实例**只在 dogfood 验证通过后手动升级到指定 commit。发布流程见 CLAUDE.md。
