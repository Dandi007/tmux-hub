...
**来源说明：** 下面「2 开发流程」「3 测试规范」「4 代码约定」三节，是从仓根 `AGENTS.md` 第 33–232 行**逐字抽取**（`sed -n '33,232p'`）移入的，**一字未改**，章节编号一并保留——`.github/pull_request_template.md` 与本仓既有约定引用的「AGENTS.md §2.1」在此仍可按同一编号定位。
**新增：** 只在文末追加「5 分支与合入（生态口径）」与「6 文档」两节。
...
2. **dev-dispatch 单的 base 禁止是 `main`**（生态宪法第十条）。
3. **不在主 checkout 上开新分支干活**；新工作一律另起 worktree。注意本仓有两处 checkout：macOS `~/code/tmux-hub`（钉已验证 commit）与 NUC `/data/code/self/tmux-hub`（跟 main），都不要直接在上面开发。
4. 本仓**没有 release 分支**。发布形态见 [CLAUDE.md](../../CLAUDE.md) 的「部署拓扑（双平台）」：macOS 走 dogfood → promote 金丝雀流程，NUC 走 `zsh deploy/instances.zsh deploy-prod`。

## 6  文档

5. 设计/规格/计划落 `docs/specs/`，硬线落 `docs/constitution/`，命名 `NNN-kebab-topic.md`，三位递增、**号码不复用**、两目录独立编号。
6. **例外：`docs/superpowers/specs/` 与 `docs/superpowers/plans/` 保持原样。** 上面 §2.1 与 `.github/pull_request_template.md` 把 `docs/superpowers/specs/YYYY-MM-DD-<slug>-design.md` 定为本仓现行强制约定；改动它需要同时改 PR 模板（CI/配置面），超出 docs-only 范围。新的每-MR 设计文档继续按 §2.1 落在那里。
7. 文档移动必须 `git mv`（保 git 历史）；迁移时不改写正文，只修被移动打断的 markdown 链接。
8. 根 [AGENTS.md](../../AGENTS.md) 是 agent 的 canonical 入口，只做导航；新增文档必须同步登记进它的文档地图。
